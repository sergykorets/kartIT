class Currency < ApplicationRecord
  ...
  def get_current_amount
    if yesterday_balance = balances.where('balances.created_at >= ?', 1.day.ago).last
      ...
      plus_amounts_exchange = buy_actions.not_canceled.after_time(yesterday_balance.created_at)
      minus_amounts = cashdesk_actions.collection.after_time(yesterday_balance.created_at)
      minus_amounts_exchange = sell_actions.not_canceled.after_time(yesterday_balance.created_at)
